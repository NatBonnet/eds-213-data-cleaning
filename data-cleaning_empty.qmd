...
unique(snowsurvey_csv$Snow_cover)

unique(snowsurvey_csv$Year)
```
...
snowsurvey_fixed <- snowsurvey_csv %>% 
  mutate(Snow_cover = if_else(Snow_cover == ".", NA, Snow_cover))
```
...
```{r}
snowsurvey_csv <- snowsurvey_fixed
```
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snow_cover = if_else(Snow_cover == "n/a", NA, Snow_cover))
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snow_cover = if_else(Snow_cover == "unk", NA, Snow_cover))
```
...
```{r}
snowsurvey_fixed <- snowsurvey_fixed %>% 
  mutate(Snow_cover = if_else(Snow_cover == "<1", "0", Snow_cover))
```
...
species_long <- species_csv %>% 
  ...
  pivot_longer(cols = !c(Year, Site, Date),
               names_to = "species",
               values_to = "presence", 
